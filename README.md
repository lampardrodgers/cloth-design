...
3. 拿到 owner 之后把 `ALLOW_SELF_SIGNUP=false` 写进 `.env` 重启，从此登录页只剩登录，别人注册不进来。
...
| 想做的事 | 怎么做 |
| --- | --- |
| 给朋友开号 | 填邮箱 + 初始密码（≥8 位）+ 角色 + 初始积分，点「创建账号」。建好就能登录，不用再开通。把邮箱和密码发给对方即可 |
| 让某人不扣积分 | 那一行的「无限」点成「∞ 已开」。该账号登录后顶栏显示 ∞，出图不计费 |
| 忘记密码 | 那一行点「改密」，输入新密码，再告诉对方 |
| 停用某人 | 「状态」改成「锁定」，他会被挡在登录之后。想留着数据又不想他用，用这个而不是删号 |
| 调积分 | 「+100 / -100」，或在「积分流水」里核对 |
| 升管理员 | 「角色」改成 admin 或 owner。管理员不受开通开关限制 |

如果 `ALLOW_SELF_SIGNUP` 还开着，别人注册进来会是「待开通」状态，登录时被拦住并提示等待；在这一行点「开通」才放行。关掉自助注册后就没有这个环节了。

出图费用二选一：用你的共享 Key（按积分计费，你在后台调分或直接开「无限」），或让对方在「账户」页填自己的图像接口 Key（AES-256-GCM 加密落库，页面只回显前 3 位和后 4 位，用自备 Key 出图不扣积分）。注意自备 Key 必须与服务端 `OPENAI_BASE_URL` 是同一家接口——Base URL 和模型名是全站统一配置。
